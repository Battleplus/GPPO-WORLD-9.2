...
- 固定 R2 world-model 在新 final-test 上 event PR-AUC 为 0.077/0.134/0.061，0.5 阈值三类 precision/recall 均为 0；done BCE 0.266，永不 done baseline 更好。R3 不宣称预测驱动触发有效。
...
- 世界模型预训练成本沿用 R2 checkpoint，R3 未重新训练世界模型；R3 证明实际消费路径和推理成本，不能把 context projection 改称新训练出的预测头。
