...
日期：2026-09-04。状态：D-01/D-02 已执行，D-03 假设判定与 D-04 设计已记录；新训练和 T-06 尚未开始。最新结果见 [D-02 最终报告](../nodes/D-02/evidence/final-report.md)，下一轮设计见 [D-04](../nodes/D-04/README.md)。
...
| D-02 | 非 Test 开发集的 residual、同一 checkpoint 的 base/adapter 动作分歧与回退原因 | 已通过：108 对/216 episodes，623 条探针记录，安全写入 0 |
| D-03 | 判定事件归因、信息未接入、异常回退与适配器优化假设 | 已记录支持/不支持/未确定；没有声称因果解释已完成 |
| D-04 | 新一轮主指标、配对 seeds、预算、新 Test、无 WM/zero-context/历史对照设计 | 设计已保存；执行和最终数据 hash 冻结仍未开始 |
